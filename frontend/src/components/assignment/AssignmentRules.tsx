...
import CreateRuleModal from '../../pages/GreateRuleModal'
...
const iconColors = [
    ...
    { iconBg: 'bg-[rgba(0,182,122,0.1)]', iconColor: 'text-[#00b67a]' },
]
...
export default function AssignmentRules() {
    ...
    const { mutate: toggle } = useMutation({
        mutationFn: ({ id, isActive }: { id: number; isActive: boolean }) =>
            updateAssignmentRule(id, { isActive }),
        ...
        <>
            <div>
                <div className="flex items-start justify-between flex-wrap gap-3 mb-4">
                    ...
                    <button onClick={() => setCreateOpen(true)} className="flex items-center gap-2 bg-[#00b67a] text-white text-sm font-medium px-4 h-10 rounded-lg">
                        <Plus size={16} />
                        New Rules
                    </button>
                </div>
                        ...
                        return (
                            ...
                            className="bg-white border border-gray-200 rounded-2xl px-6 py-8 flex items-start justify-between gap-4 cursor-pointer hover:border-[#00b67a] hover:shadow-sm transition-all">
                                <div className="flex items-start gap-4 flex-1 min-w-0">
                                    ...
                                    <div className="min-w-0">
                                        ...
                                        <p className="text-sm mt-1">
                                            Assign to: <span className="text-[#00b67a] font-medium">{r.team_name ?? '—'}</span>{' '}
                                ...
                                <div onClick={(e) => e.stopPropagation()} className="flex items-center gap-3 flex-shrink-0">
                                    ...
                                    <button
                                    onClick={(e) => { e.stopPropagation(); setEditRule(r) }}
