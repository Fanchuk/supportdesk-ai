...
export default function SlaPolicies() {
    ...
    const { mutate: toggle } = useMutation({
        mutationFn: ({ id, isActive }: { id: number; isActive: boolean }) => updateSlaPolicy(id, { isActive }),
        onMutate: async ({ id, isActive }) => {
            await qc.cancelQueries({ queryKey: ['sla-policies'] })
            const previous = qc.getQueryData(['sla-policies'])

            qc.setQueryData(['sla-policies'], (old: any[]) => old.map((p) => (p.id === id ? { ...p, isActive } : p)))

            return { previous }
        },
        onError: (_err, _vars, ctx) => qc.setQueryData(['sla-policies'], ctx?.previous),
                ...
                <div className="flex flex-col gap-4">
                    {(data as any[]).map((p) => (
                        <div
                            ...
                            className="bg-white border border-gray-200 rounded-2xl px-6 py-6 flex items-center justify-between gap-4 cursor-pointer hover:border-[#0A86F5] hover:shadow-sm transition-all">
                            <div className="flex items-center gap-4 flex-1 min-w-0">
                                <div className="w-12 h-12 rounded-[8px] flex items-center justify-center flex-shrink-0" style={{ background: 'rgba(71,222,222,0.1)' }}>
                                    <Shield size={20} className="text-[#47DEDE]" />
                                </div>
                                <div className="min-w-0 flex-1">
                                    <div className="flex items-center gap-2 flex-wrap">
                                        <p className="text-base font-medium text-black">{p.name}</p>
                                        <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${priorityStyle[p.priority]}`}>{p.priority}</span>
                                    </div>
                                    <div className="flex gap-6 mt-2 text-sm text-gray-500">
                                        <span>
                                            First response: <span className="font-medium text-[#202020]">{p.firstResponseHours ?? p.first_response_hours}h</span>
                                        </span>
                                        <span>
                                            Resolution: <span className="font-medium text-[#202020]">{p.resolutionHours ?? p.resolution_hours}h</span>
                                        </span>
                                    </div>
                                </div>
                            </div>
