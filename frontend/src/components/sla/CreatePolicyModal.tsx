...
export default function CreatePolicyModal({ open, onClose }: { open: boolean; onClose: () => void }) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault()
        if (!form.name || !form.priority || !form.firstResponseHours || !form.resolutionHours) return toast.error('Fill in all fields')
        mutate()
    }
                    ...
                    <motion.div
                        ...
                        <form onSubmit={handleSubmit} className="p-6 flex flex-col gap-4">
                            ...
                            <div className="flex gap-3 mt-2">
                                <button type="button" onClick={onClose} className="flex-1 h-10 border border-gray-200 rounded-lg text-sm text-gray-600 hover:bg-gray-50 transition-colors">
                                    Cancel
                                </button>
                                <button
                                    type="submit"
